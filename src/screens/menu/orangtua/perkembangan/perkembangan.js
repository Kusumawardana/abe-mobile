...
import {Body, Button, Header, Icon, Left, Right, Title,Text,List,ListItem,Item,Input,Subtitle,Container,Content,Footer,FooterTab} from "native-base";
...
export default function OrtuPerkembanganScreen({ navigation }) {
  ...
  const getData = (user) =>{
    if (!loading && !isListEnd){
        ...
        setLoading(true);
        const url = API_KEY.CHILDREN + from + '/' + to + '/' + user + '/' + search
        console.log(url)
        actionget(url)
            .then(value => {
  ...
  const searchData = (text) =>  {
    ...
    setLoading(true);
    setFrom(0)
    const url = API_KEY.CHILDREN + from + '/' + to + '/' + user + '/' + text
    actionget(url)
    .then(value => {
      setIsListEnd(false)
      if(text.length == 0){
  ...
  const getItem = (item) => {
    // console.log('nilai perkembangan anak:',item.anak.id);
    navigation.navigate('PerkembanganList', {data: item});
  };
